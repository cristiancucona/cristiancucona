    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8">
            <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold">Location Hierarchy</h2>
            </div>
            ...
            {isAuthorized && (
                <form onSubmit={handleCreate} className="bg-white p-6 rounded-lg shadow-sm border border-slate-200 flex gap-4 items-end">
                    <div className="flex-1">
                        ...
                        <input required value={name} onChange={e => setName(e.target.value)} className="w-full border border-slate-300 rounded-md px-3 py-2" placeholder="e.g. Main Fridge" />
                    </div>
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {locations.map(loc => (
                            <tr key={loc.id} className="hover:bg-slate-50">
                                <td className="px-6 py-4 font-mono text-slate-500">{loc.id}</td>
                        ...
                        {locations.length === 0 && (
                            <tr><td colSpan={3} className="px-6 py-12 text-center text-slate-500">No logical holding locations defined.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
